(function () {
  ...
  function handleManagementAction(action, target) {
    const deviceModal = editing => openModal(editing ? '编辑设备' : '新增设备', `<div class="proto-field"><label>设备SN</label><input ${editing ? 'value="SI1020E1112" disabled' : 'placeholder="请输入设备SN"'}></div><div class="proto-field"><label>设备名称</label><input value="${editing ? 'AOI-01' : ''}" placeholder="请输入设备名称"></div><div class="proto-field"><label>设备IP</label><input value="${editing ? '192.168.10.21' : ''}" placeholder="例如：192.168.10.21"></div><div class="proto-field"><label>所属区域</label><select><option>区域A</option><option>区域B</option><option>区域C</option><option>区域D</option></select></div><div class="proto-field"><label>设备状态</label><select><option>启用</option><option>停用</option></select></div>`, editing ? '保存修改' : '新增设备', () => toast(editing ? '设备信息已保存' : '设备已新增'));
    ...
    if (action === 'add-device') return deviceModal(false);
    if (action === 'edit-device') return deviceModal(true);
